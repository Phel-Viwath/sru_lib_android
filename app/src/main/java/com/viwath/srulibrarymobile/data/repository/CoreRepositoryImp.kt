...
class CoreRepositoryImp @Inject constructor(
    ...
    override suspend fun getStudentById(id: Long): Result<Students, DataError.Remote> {
    ...
    override suspend fun addBooks(books: List<Book>): Result<Unit, DataError.Remote> {
        return safeCall { api.addBooks(books) }
    }

    override suspend fun uploadBook(file: MultipartBody.Part): Result<Unit, DataError.Remote> {
        return safeCall { api.uploadBook(file) }
    }

    override suspend fun updateBook(book: Book): Result<Unit, DataError.Remote> {
        return safeCall { api.updateBook(book) }
    }

    override suspend fun getBooks(): Result<List<BookDto>, DataError.Remote> {
    ...
    override suspend fun moveToTrash(bookId: String): Result<Unit, DataError.Remote> {
    ...
    override suspend fun college(): Result<List<College>, DataError.Remote> {
        return safeCall { api.college() }
    }

    override suspend fun searchBook(
        keyword: String
    ): Result<List<BookDto>, DataError.Remote> {
        return safeCall { api.searchBook(keyword) }
    }

    override suspend fun borrowBook(borrow: BorrowRequest): Result<Unit, DataError.Remote> {
        return safeCall { api.activeBorrowDetails(borrow) }
    }

    override suspend fun getAllBorrowsDetail(): Result<List<BorrowDetailDto>, DataError.Remote> {
        return safeCall { api.getAllBorrowDetails() }
    }

    override suspend fun getActiveBorrowsDetail(): Result<List<BorrowDetailDto>, DataError.Remote> {
        return safeCall { api.activeBorrowDetails() }
    }

    override suspend fun searchBorrow(keyword: String): Result<List<BorrowDetailDto>, DataError.Remote> {
        return safeCall { api.searchBorrow(keyword) }
    }

    override suspend fun extendBorrow(id: BorrowId): Result<Unit, DataError.Remote> {
    ...
    override suspend fun addDonation(donationIO: DonationIO): Result<Unit, DataError.Remote> {
